...
* [`src/my_closet_frontend/`](src/my_closet_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/my_closet_frontend/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `my-closet-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/my_closet_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns my-closet-frontend.example
  (:require [my-closet-frontend.config :as config])
